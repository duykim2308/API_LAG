    ...
    public class GHN
    {
        public static int Create(List<DataObjects.GHNXML> lsArray)
        {
            ...
            try
            {
                ...
                result = conn.ExecuteNonQuery("sp_GHN_Webhook_Create", lsInput);
                conn.Commit();
            }
            catch (Exception ex) { conn.RollBack(); result = -1; Utilities.FileLog.WriteFileLog("DataAccess-->GHNWebhook-->Create::" + ex.Message); }
            finally { conn.Close(); }
            return result;
        }
    }
