...
namespace DataAccess
{
    ...
    {
        public static int Create(List<DataObjects.Insider.XML> lsArray)
        {
            int result = -1;
            ...
            try
            {
                ...
                result = conn.ExecuteNonQuery("sp_INSIDER_Webhook_Create", lsInput);
                conn.Commit();
            }
            catch (Exception ex) { conn.RollBack(); result = -1; Utilities.FileLog.WriteFileLog("DataAccess-->Insider-->Create::" + ex.Message); }
            finally { conn.Close(); }
            return result;
        }
